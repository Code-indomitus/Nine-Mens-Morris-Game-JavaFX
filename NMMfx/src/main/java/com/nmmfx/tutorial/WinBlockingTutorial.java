...
public class WinBlockingTutorial extends GameTutorial{
    public WinBlockingTutorial(Stage gameAppStage, Scene tutorialScene) {
    ...
    /**
     * Sets up the unique tutorial environment for teaching the win by blocking mechanic of the game
     */
    @Override
    protected void tutorial() {
        Intersection startingIntersection = (Intersection) boardTiles[4][6];
        startingIntersection.setFill(Color.YELLOW);
        startingIntersection.setOpacity(0.5);

        // destination intersection at coordinate (6, 10)
        Intersection destinationIntersection = (Intersection) boardTiles[2][6];
        ...
        Intersection intersection4 = (Intersection) boardTiles[2][2];
        RedPiece redPiece3 = new RedPiece(intersection4.getTileX(), intersection4.getTileY());
        this.piecesGroup.getChildren().add(redPiece3);

        Intersection intersection5 = (Intersection) boardTiles[6][2];
        BluePiece bluePiece2 = new BluePiece(intersection5.getTileX(), intersection5.getTileY());
        this.piecesGroup.getChildren().add(bluePiece2);

        Intersection intersection6 = (Intersection) boardTiles[10][2];
        ...
        Intersection intersection7 = (Intersection) boardTiles[0][6];
        RedPiece redPiece4 = new RedPiece(intersection7.getTileX(), intersection7.getTileY());
        this.piecesGroup.getChildren().add(redPiece4);

        Intersection intersection8 = (Intersection) boardTiles[0][12];
        BluePiece bluePiece4 = new BluePiece(intersection8.getTileX(), intersection8.getTileY());
            ...
            if ((newX >= 0 && newX <= 12) && (newY >= 0 && newY <= 12)) {
                if (BoardUtility.isCoordinateIntersection(newX, newY)) {
                    ...
                    if (newX == destinationIntersection.getTileX() && newY == destinationIntersection.getTileY()) {
                        // Indicate to the user that they have successfully completed this tutorial
                        ...
                        this.infoLabel.setText("WELL DONE! RED BLOCKED! BLUE WINS!");
                        this.infoLabel.setTextFill(Color.DARKVIOLET);
                        infoLabel.relocate(105, 10);
    ...
    /**
     * Creates the image for displaying instructions for the winning by blocking tutorial
     * @return ImageView container for image of instructions
     */
    @Override
    protected ImageView createInstructions() {
        ImageView winBlockingInstructions = new ImageView(new Image( "win_block_tutorial.png" ));
        winBlockingInstructions.setTranslateX(550);
        winBlockingInstructions.setTranslateY(25);

        return winBlockingInstructions;
    }
}
